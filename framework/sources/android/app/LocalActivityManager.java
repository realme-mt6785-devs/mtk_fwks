        ...
        if (r.curState != 0 && r.curState != 5) {
            if (r.curState == 1) {
                ...
                if (r.activity != null) {
                    ...
                    this.mActivityThread.handleStartActivity(clientRecord, pendingActions, null);
                    r.curState = 3;
                    if (desiredState == 4) {
                        this.mActivityThread.performResumeActivity(clientRecord, true, "moveToState-INITIALIZING");
                        r.curState = 4;
                        return;
                    }
                    return;
            ...
            switch (r.curState) {
                case 2:
                    if (desiredState == 3) {
                        this.mActivityThread.performRestartActivity(clientRecord2, true);
                        r.curState = 3;
                    }
                    if (desiredState == 4) {
                        this.mActivityThread.performRestartActivity(clientRecord2, true);
                        this.mActivityThread.performResumeActivity(clientRecord2, true, "moveToState-CREATED");
                        r.curState = 4;
                        return;
                    }
                    return;
                case 3:
                    if (desiredState == 4) {
                        this.mActivityThread.performResumeActivity(clientRecord2, true, "moveToState-STARTED");
                        r.instanceState = null;
                        r.curState = 4;
                    }
                    if (desiredState == 2) {
