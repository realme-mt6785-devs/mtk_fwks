...
public class Notification implements Parcelable {
    ...
    public static class Builder {
        private static final String EXTRA_BIG_CONTENT_NULL_KEY = "EXTRA_VERSION_CODES_RESULT_NULL";
        ...
        public RemoteViews applyStandardTemplate(int resId, StandardTemplateParams p, TemplateBindResult result) {
            ...
            if (p.text != null && p.text.length() != 0 && (!showProgress || p.mAllowTextWithProgress)) {
                ...
                hasSecondLine = true;
            } else if (p.mTextViewId != 16909529) {
        ...
        private void calculateRightIconDimens(Icon rightIcon, boolean isPromotedPicture, TemplateBindResult result) {
            ...
            if (rightIcon == null || ((!isPromotedPicture && this.mContext.getApplicationInfo().targetSdkVersion < 31) || (drawable = rightIcon.loadDrawable(this.mContext)) == null || (iconWidth = drawable.getIntrinsicWidth()) <= (iconHeight = drawable.getIntrinsicHeight()) || iconHeight <= 0)) {
                ...
                viewWidthDp = Math.min((iconWidth * viewHeightDp) / iconHeight, maxViewWidthDp);
            }
        ...
        private void bindLargeIcon(RemoteViews contentView, StandardTemplateParams p, TemplateBindResult result) {
            ...
            boolean baseMedia = contentView.getLayoutId() == 17367222;
            if (rightIcon == null || baseMedia) {
        ...
        public RemoteViews createBigContentView() {
            ...
            if (result == null) {
                if (this.mActions.size() == 0) {
                    getExtras().putBoolean(EXTRA_BIG_CONTENT_NULL_KEY, true);
                } else {
                    getExtras().putBoolean(EXTRA_BIG_CONTENT_NULL_KEY, false);
                }
        ...
        public RemoteViews createHeadsUpContentView(boolean increasedHeight) {
            ...
            } else if (fullyCustomViewRequiresDecoration(false)) {
        ...
        public RemoteViews makePublicContentView(boolean isLowPriority) {
            ...
            RemoteViews view = applyStandardTemplate(getBaseLayoutResource(), params, null);
        ...
        private RemoteViews generateActionButton(Action action, boolean emphasizedMode, StandardTemplateParams p) {
            ...
            if (emphasizedMode) {
                ...
                i = tombstone ? getActionTombstoneLayoutResource() : getActionLayoutResource();
            }
            ...
            if (emphasizedMode) {
                ...
                if (isLegacy()) {
                    ...
                    title = ensureColorSpanContrast(title2, notifBackgroundColor, outResultColor);
                }
                button.setTextViewText(R.id.action0, processTextSpans(title));
                if (outResultColor[0] == null) {
        ...
        private static CharSequence ensureColorSpanContrast(CharSequence charSequence, int background, ColorStateList[] outResultColor) {
            Object[] spans;
            if (!(charSequence instanceof Spanned)) {
                return charSequence;
            }
            ...
            int i = 0;
            while (i < length) {
                Object span = spans2[i];
                Object resultSpan = span;
                int spanStart = ss.getSpanStart(span);
                int spanEnd = ss.getSpanEnd(span);
                if (spanEnd - spanStart == charSequence.length()) {
                    fullLength = true;
                }
                ...
                if (resultSpan instanceof TextAppearanceSpan) {
                    ...
                    if (resultSpan instanceof ForegroundColorSpan) {
                        ...
                        int foregroundColor2 = ContrastColorUtil.ensureLargeTextContrast(foregroundColor, background, isBgDark2);
                        if (fullLength) {
                            fullLength = false;
                            outResultColor[0] = ColorStateList.valueOf(foregroundColor2);
                            resultSpan = null;
                        } else {
                            fullLength = false;
                            resultSpan = new ForegroundColorSpan(foregroundColor2);
                        }
                    } else {
                        fullLength = false;
                        resultSpan = span;
        ...
        private void processSmallIconColor(Icon smallIcon, RemoteViews contentView, StandardTemplateParams p) {
            ...
            if (!isLegacy() && SystemProperties.getBoolean("persist.oppo.ctsversion", false)) {
                colorable = true;
            }
            if (smallIcon != null && !colorable) {
                if (smallIcon.getType() == 2) {
                    colorable = getColorUtil().isGrayscaleIcon(getPackageContext(smallIcon.getResPackage()), smallIcon);
                } else {
                    colorable = getColorUtil().isGrayscaleIcon(this.mContext, smallIcon);
                }
            }
        ...
        private void processLargeLegacyIcon(Icon largeIcon, RemoteViews contentView, StandardTemplateParams p) {
            if (largeIcon != null && isLegacy()) {
                Context context = largeIcon.getType() == 2 ? getPackageContext(largeIcon.getResPackage()) : this.mContext;
                if (getColorUtil().isGrayscaleIcon(context, largeIcon)) {
                    int color = getSmallIconColor(p);
        ...
        private Context getPackageContext(String pkg) {
            ...
            Context context = this.mPkgContext;
            if (context == null || !pkg.equals(context.getPackageName())) {
                try {
                    ApplicationInfo ai = this.mContext.getPackageManager().getApplicationInfoAsUser(pkg, 8192, this.mContext.getUserId());
                    this.mPkgContext = this.mContext.createApplicationContext(ai, 4);
                } catch (PackageManager.NameNotFoundException e) {
                    Log.e(Notification.TAG, "pkg: " + pkg, e);
                    this.mPkgContext = this.mContext;
                }
            }
            return this.mPkgContext;
        }
    ...
    public static void buildCustomContentIntoTemplate(Context context, RemoteViews template, RemoteViews customContent, StandardTemplateParams p, TemplateBindResult result) {
        ...
        if (customContent != null) {
            ...
            RemoteViews remoteViews = new BuilderRemoteViews(context.getApplicationInfo(), R.layout.notification_template_custom_container_parent);
            remoteViews.addView(R.id.custom_container, customContent2, 0);
            template.addView(R.id.notification_main_column, remoteViews, 0);
            template.addFlags(1);
            childIndex = 0;
        }
    ...
    public static class MessagingStyle extends Style {
        ...
        public IMessagingStyleExt mMessagingStyleExt;
        Icon mShortcutIcon;
        ...
        MessagingStyle() {
            ...
            this.mMessagingStyleExt = (IMessagingStyleExt) ExtLoader.type(IMessagingStyleExt.class).base(this).create();
        }
        ...
        public MessagingStyle setConversationType(int conversationType) {
            this.mConversationType = this.mMessagingStyleExt.getConversationType(conversationType);
            return this;
        }
        ...
        public boolean areNotificationsVisiblyDifferent(Style other) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                String newSender = String.valueOf(charSequence2);
        ...
        private RemoteViews makeMessagingView(int viewType) {
            ...
            int conversationType = this.mMessagingStyleExt.getConversationType(this.mConversationType);
            this.mConversationType = conversationType;
            boolean isConversationLayout = conversationType != 0;
            boolean isImportantConversation = conversationType == 2;
            if (!TextUtils.isEmpty(((Style) this).mBigContentTitle)) {
                conversationTitle = ((Style) this).mBigContentTitle;
            } else {
                conversationTitle = this.mConversationTitle;
            ...
            StandardTemplateParams text = this.mBuilder.mParams.reset().viewType(viewType).highlightExpander(isConversationLayout).hideProgress(true).title(conversationTitle).text(null);
            if (hideRightIcons || isOneToOne) {
                z = true;
            }
            StandardTemplateParams p = text.hideRightIcon(z).headerTextSecondary(conversationTitle);
            RemoteViews contentView = this.mBuilder.applyStandardTemplateWithActions(isConversationLayout ? this.mBuilder.getConversationLayoutResource() : this.mBuilder.getBigMessagingLayoutResource(), p, bindResult);
    ...
    public static class MediaStyle extends Style {
        ...
        protected RemoteViews makeMediaContentView(RemoteViews customContent) {
            ...
            if (numActionsToShow <= numActions) {
                StandardTemplateParams p = this.mBuilder.mParams.reset().viewType(StandardTemplateParams.VIEW_TYPE_NORMAL).hideProgress(true).fillTextsFrom(this.mBuilder);
                TemplateBindResult result = new TemplateBindResult();
                RemoteViews template = this.mBuilder.applyStandardTemplate(R.layout.notification_template_material_media, p, result);
    ...
    public static class CallStyle extends Style {
        ...
        private Action makeAnswerAction() {
            ...
            return makeAction(z ? R.drawable.ic_call_answer_video : R.drawable.ic_call_answer, z ? R.string.call_notification_answer_video_action : 17039814, this.mAnswerButtonColor, R.color.call_notification_answer_color, pendingIntent);
        }
    ...
    public static class TemplateBindResult {
        ...
        public class MarginSet {
            ...
            public void applyToView(RemoteViews views, int viewId, float extraMarginDp) {
                ...
                } else if (viewId != 16909529 && viewId != 16908794) {
                    views.setViewLayoutMargin(viewId, 5, marginEndDp, 1);
                } else if (this.mValueIfGone == 0.0f) {
        ...
        public void resolvePalette(Context ctx, int rawColor, boolean isColorized, boolean nightMode) {
            ...
            if (this.mPaletteIsForRawColor != rawColor || this.mPaletteIsForColorized != isColorized || this.mPaletteIsForNightMode != nightMode) {
                ...
                if (isColorized) {
                    ...
                    ta = obtainDayNightAttributes(ctx, new int[]{R.attr.colorSurface, 16844002, 16842806, 16842808, 16843829, R.attr.colorAccentSecondary, 16844099, 16843820});
                    if (nightMode) {
                        i = -16777216;
                    }
                    try {
                        ...
                        this.mErrorColor = getColor(ta, 6, 1);
                        this.mRippleAlpha = Color.alpha(getColor(ta, 7, 872415231));
                        if (ta != null) {
                            ta.close();
                        }
                        this.mPrimaryAccentColor = 1;
                        this.mContrastColor = calculateContrastColor(ctx, rawColor, 1, this.mBackgroundColor, nightMode);
                        ...
                        int i3 = this.mContrastColor;
                        this.mPrimaryAccentColor = i3;
                        this.mSecondaryAccentColor = i3;
