...
public class Camera {
    ...
    private final Object mShutterSoundLock;
    ...
    public static Camera open(int cameraId) {
        if (!CameraExtPlugin.interceptOpenWithCameraId.call(new Object[0]).booleanValue()) {
            return new Camera(cameraId);
        }
        throw new RuntimeException("Open camera denied");
    }

    public static Camera open() {
        if (CameraExtPlugin.interceptOpen.call(new Object[0]).booleanValue()) {
            return null;
        }
    ...
    public static Camera openLegacy(int cameraId, int halVersion) {
        ...
        } else if (!CameraExtPlugin.interceptOpenLegacy.call(new Object[0]).booleanValue()) {
            return new Camera(cameraId);
        } else {
            throw new RuntimeException("Open camera denied");
        }
    ...
    private int cameraInit(int cameraId) {
        ...
        return native_setup(new WeakReference(this), cameraId, this.mCamExt.getComponentName());
    }

    Camera(int cameraId) {
        this.mFaceDetectionRunning = false;
        this.mAutoFocusCallbackLock = new Object();
        this.mShutterSoundLock = new Object();
        this.mHasAppOpsPlayAudio = true;
        this.mShutterSoundEnabledFromApp = true;
        this.mCamExt = CameraExtPlugin.getInstance.call(new Object[0]);
        int err = cameraInit(cameraId);
        if (!checkInitErrors(err)) {
            initAppOps();
            long time = System.currentTimeMillis();
            this.mCamExt.extendCamera(cameraId, time);
    ...
    public final void release() {
        ...
        releaseAppOps();
        long time = System.currentTimeMillis();
        this.mCamExt.extendRelease(time);
    }
    ...
    public final void stopPreview() {
        ...
        try {
            ...
            Log.e(TAG, "failure in getParameters");
        }
        this.mCamExt.extendstopPreview(parameters);
        _stopPreview();
        this.mFaceDetectionRunning = false;
        this.mShutterCallback = null;
        this.mRawImageCallback = null;
            ...
            switch (msg.what) {
                ...
                case 1024:
                    if (Camera.this.mFaceListener != null) {
                        ...
                        if (faces != null) {
                            Camera.this.mCamExt.extendhandleMessage(faces.length);
                            return;
                        }
                        return;
                    }
