...
public class SystemSensorManager extends SensorManager {
    ...
    public static ISystemSensorManagerExt mSystemSensorManagerExt = SystemSensorManagerExtPlugin.constructor.newInstance();
    ...
    private static native void nativesensorServiceCommonInterface(long j, String str, int i, int i2);
    ...
    protected boolean registerListenerImpl(SensorEventListener listener, Sensor sensor, int delayUs, Handler handler, int maxBatchReportLatencyUs, int reservedFlags) {
        ...
        if (listener == null || sensor == null) {
            ...
            return false;
        } else if (mSystemSensorManagerExt.registerListenerImplHook(this.mContext, listener, sensor, delayUs)) {
            return false;
        } else {
    ...
    protected void unregisterListenerImpl(SensorEventListener listener, Sensor sensor) {
        boolean result;
        mSystemSensorManagerExt.unregisterListenerImplHook(listener, sensor);
    ...
    public static abstract class BaseEventQueue {
        ...
        BaseEventQueue(Looper looper, SystemSensorManager manager, int mode, String packageName) {
            ...
            this.mNativeSensorEventQueue = nativeInitBaseEventQueue(manager.mNativeInstance, new WeakReference(this), looper.getQueue(), packageName, mode, manager.mContext.getOpPackageName(), manager.mContext.getAttributionTag());
        ...
        private int enableSensor(Sensor sensor, int rateUs, int maxBatchReportLatencyUs) {
            if (this.mNativeSensorEventQueue == 0) {
                ...
                throw new NullPointerException();
            } else if (!this.mManager.isSensorInCappedSet(sensor.getType()) || rateUs >= 5000 || !this.mManager.mIsPackageDebuggable || this.mManager.mHasHighSamplingRateSensorsPermission || !Compatibility.isChangeEnabled((long) SystemSensorManager.CHANGE_ID_SAMPLING_RATE_SENSORS_PERMISSION)) {
        ...
        protected int dispatchSensorEvent(int handle, float[] values, int accuracy, long timestamp, int cmdArgs) {
            return 0;
        }

        protected void dispatchSensorEventWithType(int handle, float[] values, int accuracy, long timestamp, int type, int cmdArgs) {
            int res = dispatchSensorEvent(handle, values, accuracy, timestamp, cmdArgs);
            SystemSensorManager.mSystemSensorManagerExt.dispatchSensorEventEnd(res, handle, values, accuracy, type, cmdArgs, getListener());
        }
    ...
    public static final class SensorEventQueue extends BaseEventQueue {
        ...
        protected void dispatchSensorEvent(int handle, float[] values, int inAccuracy, long timestamp) {
            dispatchSensorEvent(handle, values, inAccuracy, timestamp, 0);
        }

        @Override // android.hardware.SystemSensorManager.BaseEventQueue
        protected String getListener() {
            return "SensorEventQueue_" + this.mPackageName;
        }
    ...
    public static final class TriggerEventQueue extends BaseEventQueue {
        ...
        protected void dispatchSensorEvent(int handle, float[] values, int accuracy, long timestamp) {
            dispatchSensorEvent(handle, values, accuracy, timestamp, 0);
        }

        @Override // android.hardware.SystemSensorManager.BaseEventQueue
        protected String getListener() {
            return "TriggerEventQueue_" + this.mPackageName;
        }
    ...
    @Override // android.hardware.SensorManager
    protected void sensorServiceCommonInterface(String funcName, int flag, int state) {
        Log.v("SensorManager", "call sensorServiceCommonInterface name: " + funcName);
        nativesensorServiceCommonInterface(this.mNativeInstance, funcName, flag, state);
    }
}
