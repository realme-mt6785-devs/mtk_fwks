...
public class BiometricTestSession implements AutoCloseable {
    ...
    public BiometricTestSession(Context context, int sensorId, TestSessionProvider testSessionProvider) throws RemoteException {
        ITestSessionCallback.Stub stub = new ITestSessionCallback.Stub() { // from class: android.hardware.biometrics.BiometricTestSession.1
            ...
            public void onCleanupStarted(int userId) {
                String tag = BiometricTestSession.this.getTag();
                Log.d(tag, "onCleanupStarted, sensor: " + BiometricTestSession.this.mSensorId + ", userId: " + userId);
            }

            @Override // android.hardware.biometrics.ITestSessionCallback
            public void onCleanupFinished(int userId) {
                String tag = BiometricTestSession.this.getTag();
                Log.d(tag, "onCleanupFinished, sensor: " + BiometricTestSession.this.mSensorId + ", userId: " + userId + ", remaining users: " + BiometricTestSession.this.mUsersCleaningUp.size());
    ...
    private void setTestHalEnabled(boolean enabled) {
        try {
            String tag = getTag();
            Log.w(tag, "setTestHalEnabled, sensor: " + this.mSensorId + " enabled: " + enabled);
    ...
    public void close() {
        String tag = getTag();
        Log.d(tag, "Close, mTestedUsers size; " + this.mTestedUsers.size());
        ...
        if (!this.mUsersCleaningUp.isEmpty()) {
            String tag2 = getTag();
            Log.e(tag2, "Cleanup not finished before shutdown - pending: " + this.mUsersCleaningUp.size());
        }
