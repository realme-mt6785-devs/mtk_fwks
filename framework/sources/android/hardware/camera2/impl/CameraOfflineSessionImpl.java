...
public class CameraOfflineSessionImpl extends CameraOfflineSession implements IBinder.DeathRecipient {
                                        ...
                                        Runnable resultDispatch3 = new Runnable() { // from class: android.hardware.camera2.impl.CameraOfflineSessionImpl.CameraDeviceCallbacks.5
                                            ...
                                            public void run() {
                                                ...
                                                if (!(CameraOfflineSessionImpl.this.isClosed() || callback == null)) {
                                                    if (holder.hasBatchedOutputs()) {
                                                        for (int i = 0; i < holder.getRequestCount(); i++) {
                                                            resultCopy.set((CaptureResult.Key<CaptureResult.Key<Long>>) CaptureResult.SENSOR_TIMESTAMP, (CaptureResult.Key<Long>) Long.valueOf(sensorTimestamp - (((subsequenceId - i) * 1000000000) / ((Integer) fpsRange.getUpper()).intValue())));
                                                            CameraMetadataNative resultLocal = new CameraMetadataNative(resultCopy);
    ...
    /* JADX WARN: Unsupported multi-entry loop pattern (BACK_EDGE: B:55:? -> B:46:0x00cd). Please submit an issue!!! */
