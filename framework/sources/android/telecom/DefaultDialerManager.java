...
public class DefaultDialerManager {
    ...
    public static String getDefaultDialerApplication(Context context, int user) {
        long identity = Binder.clearCallingIdentity();
        try {
            return (String) CollectionUtils.firstOrNull((List<Object>) ((RoleManager) context.getSystemService(RoleManager.class)).getRoleHoldersAsUser("android.app.role.DIALER", UserHandle.of(user)));
        } finally {
            Binder.restoreCallingIdentity(identity);
        }
    }
