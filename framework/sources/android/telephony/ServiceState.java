...
public class ServiceState implements Parcelable {
    ...
    protected static final String EXTRA_SERVICE_STATE = "android.intent.extra.SERVICE_STATE";
    ...
    protected final List<NetworkRegistrationInfo> mNetworkRegistrationInfos;
    ...
    public static ServiceState makeServiceState(Parcel in) {
        if (SystemProperties.get("ro.vendor.mtk_telephony_add_on_policy", "0").equals("0")) {
            try {
                Class<?> clazz = Class.forName("mediatek.telephony.MtkServiceState");
                Constructor clazzConstructfunc = clazz.getConstructor(Parcel.class);
                clazzConstructfunc.setAccessible(true);
                ServiceState sInstance = (ServiceState) clazzConstructfunc.newInstance(in);
                return sInstance;
            } catch (IllegalAccessException e) {
                ServiceState sInstance2 = new ServiceState(in);
                return sInstance2;
            } catch (InstantiationException e2) {
                ServiceState sInstance3 = new ServiceState(in);
                return sInstance3;
            } catch (InvocationTargetException e3) {
                ServiceState sInstance4 = new ServiceState(in);
                return sInstance4;
            } catch (Exception e4) {
                ServiceState sInstance5 = new ServiceState(in);
                return sInstance5;
            }
        } else {
            ServiceState sInstance6 = new ServiceState(in);
            return sInstance6;
        }
    }
}
