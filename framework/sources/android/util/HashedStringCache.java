    ...
    private void populateSaltValues(Context context, String tag, int saltExpirationDays) {
        synchronized (this.mPreferenceLock) {
            ...
            if (this.mSalt == null || needsNewSalt) {
                ...
                String saltString = sharedPreferences.getString(tag + HASH_SALT, null);
                SharedPreferences sharedPreferences2 = this.mSharedPreferences;
                int i = sharedPreferences2.getInt(tag + HASH_SALT_GEN, 0);
                this.mSaltGen = i;
                if (saltString == null || needsNewSalt) {
                    ...
                    SharedPreferences.Editor putString = edit.putString(tag + HASH_SALT, saltString);
                    SharedPreferences.Editor putInt = putString.putInt(tag + HASH_SALT_GEN, this.mSaltGen);
                    putInt.putLong(tag + HASH_SALT_DATE, System.currentTimeMillis()).apply();
                }
