...
public class InsetsSource implements Parcelable {
    ...
    private IInsetsSourceExt mInsetsSourceExt;
    ...
    public InsetsSource(int type) {
        ...
        this.mInsetsSourceExt = (IInsetsSourceExt) ExtLoader.type(IInsetsSourceExt.class).base(this).create();
        this.mType = type;
    ...
    public InsetsSource(InsetsSource other, boolean isFormInsetsAnimation) {
        this(other);
        this.mInsetsSourceExt.setFormInsetsAnimation(isFormInsetsAnimation);
    }
    ...
    private Insets calculateInsets(Rect relativeFrame, Rect frame, boolean ignoreVisibility) {
        ...
        if (relativeFrame.isEmpty() || this.mInsetsSourceExt.shouldGetIntersection(getType())) {
        ...
        if (this.mTmpFrame.width() == relativeFrame.width()) {
            ...
            if (this.mTmpFrame.bottom == relativeFrame.bottom) {
                Insets navInsets = this.mInsetsSourceExt.getNavBarInsets(getType(), this.mTmpFrame);
                if (navInsets != null) {
                    return navInsets;
                }
                return Insets.of(0, 0, 0, this.mTmpFrame.height());
            } else if (this.mTmpFrame.top == 0) {
                return Insets.of(0, this.mTmpFrame.height(), 0, 0);
            }
