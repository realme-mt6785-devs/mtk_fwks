...
public class ScaleGestureDetector {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        for (int i2 = 0; i2 < count; i2++) {
            if (skipIndex != i2) {
                devSumX2 += Math.abs(event.getX(i2) - focusX);
                devSumY += Math.abs(event.getY(i2) - focusY);
            }
        }
        float devY2 = devSumY / div;
        float spanX = (devSumX2 / div) * 2.0f;
        float devX = devY2 * 2.0f;
        if (inAnchoredScaleMode()) {
            devSumX = devX;
        } else {
            devSumX = (float) Math.hypot(spanX, devX);
        }
        boolean wasInProgress = this.mInProgress;
        this.mFocusX = focusX;
        this.mFocusY = focusY;
        if (!inAnchoredScaleMode() && this.mInProgress && (devSumX < this.mMinSpan || configChanged)) {
            this.mListener.onScaleEnd(this);
            this.mInProgress = false;
            this.mInitialSpan = devSumX;
        }
        if (configChanged) {
            ...
            this.mCurrSpan = devSumX;
            this.mPrevSpan = devSumX;
            this.mInitialSpan = devSumX;
        }
        int minSpan = inAnchoredScaleMode() ? this.mSpanSlop : this.mMinSpan;
        if (this.mInProgress || devSumX < minSpan) {
            devY = focusX;
        } else {
            if (!wasInProgress && Math.abs(devSumX - this.mInitialSpan) <= this.mSpanSlop) {
                devY = focusX;
            }
            this.mCurrSpanX = spanX;
            this.mPrevSpanX = spanX;
