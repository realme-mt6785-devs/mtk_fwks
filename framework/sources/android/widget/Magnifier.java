...
public final class Magnifier {
    ...
    private void obtainContentCoordinates(float xPosInView, float yPosInView) {
        ...
        if (this.mView instanceof SurfaceView) {
            ...
            zoomCenterX = Math.round(xPosInView + iArr2[0]);
        ...
        for (int i2 = this.mLeftContentBound; i2 >= 0; i2--) {
            resolvedLeft = Math.max(resolvedLeft, bounds[i2].left);
        }
        int resolvedTop = Integer.MIN_VALUE;
        for (int i3 = this.mTopContentBound; i3 >= 0; i3--) {
            resolvedTop = Math.max(resolvedTop, bounds[i3].top);
        }
        int resolvedRight = Integer.MAX_VALUE;
        for (int i4 = this.mRightContentBound; i4 >= 0; i4--) {
            resolvedRight = Math.min(resolvedRight, bounds[i4].right);
        }
        int resolvedBottom = Integer.MAX_VALUE;
        for (int i5 = this.mBottomContentBound; i5 >= 0; i5--) {
            resolvedBottom = Math.min(resolvedBottom, bounds[i5].bottom);
        }
        ...
        if (this.mIsFishEyeStyle) {
            i = Math.max(resolvedLeft2, Math.min(zoomCenterX, resolvedRight2));
        } else {
            int i6 = this.mSourceWidth;
            i = Math.max((i6 / 2) + resolvedLeft2, Math.min(zoomCenterX, resolvedRight2 - (i6 / 2)));
        }
        point.x = i;
        Point point2 = this.mClampedCenterZoomCoords;
        int i7 = this.mSourceHeight;
        point2.y = Math.max((i7 / 2) + resolvedTop2, Math.min(zoomCenterY, resolvedBottom2 - (i7 / 2)));
    }
