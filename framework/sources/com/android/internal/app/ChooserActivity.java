...
public class ChooserActivity extends ResolverActivity implements ChooserListAdapter.ChooserListCommunicator, SelectableTargetInfo.SelectableTargetInfoCommunicator {
                ...
                switch (msg.what) {
                    ...
                    case 2:
                        if (!ChooserActivity.this.isFinishing()) {
                            ...
                            ChooserActivity.this.mResolverActivityExt.setCornerRadius(ChooserActivity.this.getApplicationContext(), imageView);
                            return;
                        }
                        return;
                    default:
        ...
        public void maybeHideContentPreview() {
            if (!this.mAtLeastOneLoaded) {
                if (this.mHideParentOnFail) {
                    ...
                    collapseParentView();
                    ChooserActivity.this.mResolverActivityExt.hookmaybeHideContentPreview();
            ...
            if (ChooserActivity.this.mChooserMultiProfilePagerAdapter.getActiveListAdapter() != null && !ChooserActivity.this.isDestroyed()) {
                switch (msg.what) {
                    case 1:
                        ...
                        if (!(sri.resultTargets == null || (adapterForUserHandle = ChooserActivity.this.mChooserMultiProfilePagerAdapter.getListAdapterForUserHandle(sri.userHandle)) == null)) {
                    ...
                    case 4:
                        ...
                        if (resultInfo.resultTargets != null && (adapterForUserHandle2 = ChooserActivity.this.mChooserMultiProfilePagerAdapter.getListAdapterForUserHandle(resultInfo.userHandle)) != null) {
    ...
    */
    ...
    public AbstractMultiProfilePagerAdapter createMultiProfilePagerAdapter(Intent[] initialIntents, List<ResolveInfo> rList, boolean filterLastUsed) {
        ...
        return !this.mResolverActivityExt.isOriginUi() ? super.createMultiProfilePagerAdapter(initialIntents, rList, filterLastUsed) : this.mChooserMultiProfilePagerAdapter;
    }
    ...
    public void onCopyButtonClicked(View v) {
        ...
        if (Intent.ACTION_SEND.equals(action)) {
            ...
            if (!this.mResolverActivityExt.hookonCopyButtonClicked(streams)) {
                ...
                ClipData clipData2 = this.mResolverActivityExt.getclipData(targetIntent);
                clipData = clipData2;
                if (clipData2 == null) {
                    return;
                }
    ...
    protected ComponentName getNearbySharingComponent() {
        if (this.mResolverActivityExt.hookgetNearbySharingComponent()) {
            return null;
        }
    ...
    private Button createCopyButton() {
        Button b = createActionButton(getDrawable(R.drawable.ic_menu_copy_material), getString(17039361), new ChooserActivity$$ExternalSyntheticLambda1(this));
    ...
    private ViewGroup displayTextContentPreview(Intent targetIntent, LayoutInflater layoutInflater, ViewGroup parent) {
        ...
        if (this.mResolverActivityExt.isOriginUi()) {
            ...
            contentPreviewLayout = this.mResolverActivityExt.hookdisplayTextContentPreview(targetIntent, layoutInflater, parent, new ChooserActivity$$ExternalSyntheticLambda1(this));
        }
    ...
    private ViewGroup displayImageContentPreview(Intent targetIntent, LayoutInflater layoutInflater, ViewGroup parent) {
        ViewGroup contentPreviewLayout = this.mResolverActivityExt.getDisplayImageContentPreview(layoutInflater, parent);
        ...
        if (this.mResolverActivityExt.isOriginUi()) {
            ...
            this.mResolverActivityExt.addNearbyAction(contentPreviewLayout, targetIntent);
        }
    ...
    private ViewGroup displayFileContentPreview(Intent targetIntent, LayoutInflater layoutInflater, ViewGroup parent) {
        ViewGroup contentPreviewLayout = this.mResolverActivityExt.getDisplayFileContentPreview(layoutInflater, parent);
        ViewGroup actionRow = (ViewGroup) contentPreviewLayout.findViewById(R.id.chooser_action_row);
        if (this.mResolverActivityExt.isOriginUi()) {
            addActionButton(actionRow, createNearbyButton(targetIntent));
        } else {
        ...
        if (Intent.ACTION_SEND.equals(action)) {
            ...
            if (uriCount == 0) {
                ...
                int i = uriCount - 1;
                String fileName = this.mResolverActivityExt.getChooserPreFileName(getApplicationContext(), uriCount, fileInfo.name);
                ...
                this.mResolverActivityExt.setChooserPreFileSingleIconView(R.drawable.ic_file_copy, fileIconView, false, null, false, null, null);
            }
        }
        return contentPreviewLayout;
    }
    ...
    private void loadFileUriIntoView(Uri uri, View parent) {
        ...
        fileIconView.setVisibility(0);
        if (this.mResolverActivityExt.isOriginUi() || findPreferredContentPreview(uri, getContentResolver()) == 2) {
            this.mResolverActivityExt.setChooserPreFileSingleIconView(R.drawable.chooser_file_generic, fileIconView, true, fileNameView, true, fileInfo.name, uri);
            return;
        }
        ...
        contentPreviewCoordinator2.loadUriIntoView(R.id.content_preview_file_icon, uri, 0);
    }
    ...
    public void addUseDifferentAppLabelIfNecessary(ResolverListAdapter adapter) {
        ...
        if (chooserTargetArr != null && chooserTargetArr.length > 0) {
            this.mChooserMultiProfilePagerAdapter.getActiveListAdapter().addServiceResults(null, Lists.newArrayList(this.mCallerChooserTargets), 0, null, this.mServiceConnections);
        }
    }
    ...
    protected void queryTargetServices(ChooserListAdapter adapter) {
        ...
        String perm = "android.permission.BIND_CHOOSER_TARGET_SERVICE";
                        ...
                        if (this.mServicesRequested.contains(requestedItem)) {
                            ...
                            try {
                                ...
                                if (!perm.equals(perm2)) {
                                    Log.w(TAG, "ChooserTargetService " + serviceComponent + " does not require permission " + perm + " - this service will not be queried for ChooserTargets. add android:permission=\"" + perm + "\" to the <service> tag for " + serviceComponent + " in the manifest.");
                                    perm = perm;
                                } else {
    ...
    public void onListRebuilt(ResolverListAdapter listAdapter) {
        if (!this.mResolverActivityExt.isOriginUi()) {
            super.onListRebuilt(listAdapter);
            return;
        }
        setupScrollListener();
    ...
    private void setupScrollListener() {
        if (this.mResolverDrawerLayout != null) {
            ...
            this.mResolverActivityExt.setChooserHeadBackground(elevatedView);
    ...
    private boolean shouldShowStickyContentPreviewNoOrientationCheck() {
        return shouldShowTabs() && this.mMultiProfilePagerAdapter.getListAdapterForUserHandle(UserHandle.of(UserHandle.myUserId())).getCount() > 0 && isSendAction(getTargetIntent());
    }
    ...
    public final class ChooserGridAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
            ...
            for (final int i = 0; i < columnCount; i++) {
                ...
                if (!isDirectShare) {
                    v.setOnLongClickListener(new View.OnLongClickListener() { // from class: com.android.internal.app.ChooserActivity$ChooserGridAdapter$$ExternalSyntheticLambda1
                        @Override // android.view.View.OnLongClickListener
                        public final boolean onLongClick(View view) {
                            return ChooserActivity.ChooserGridAdapter.this.lambda$loadViewsIntoGroup$0$ChooserActivity$ChooserGridAdapter(holder, i, view);
                        }
                    });
                }
                holder.addView(i, v);
        ...
        public void handleScroll(View v, int y, int oldy) {
            ...
            if (directShareViewHolder != null && canExpandDirectShare) {
                directShareViewHolder.handleScroll(ChooserActivity.this.mChooserMultiProfilePagerAdapter.getActiveAdapterView(), y, oldy, ChooserActivity.this.getMaxTargetsPerRow());
            }
        }
        ...
        public void handleScroll(RecyclerView view, int y, int oldy, int maxTargetsPerRow) {
            ...
            if (notExpanded) {
                if (!this.mHideDirectShareExpansion) {
                    ChooserListAdapter adapter = ChooserActivity.this.mChooserMultiProfilePagerAdapter.getActiveListAdapter();
    ...
    public static class RoundedRectImageView extends ImageView {
        ...
        public RoundedRectImageView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
            ...
            if (this.mContext instanceof ChooserActivity) {
                ((ChooserActivity) this.mContext).mResolverActivityExt.setCustomRoundImage(this.mRoundRectPaint, this.mTextPaint, this.mOverlayPaint);
            }
        ...
        public void setExtraImageCount(int count) {
            if (count <= 0) {
                this.mExtraImageCount = null;
            } else if (!(this.mContext instanceof ChooserActivity) || !((ChooserActivity) this.mContext).mResolverActivityExt.isOriginUi()) {
