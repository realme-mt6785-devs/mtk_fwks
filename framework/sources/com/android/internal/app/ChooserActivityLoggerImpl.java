...
public class ChooserActivityLoggerImpl implements ChooserActivityLogger {
    ...
    public void logShareStarted(int eventId, String packageName, String mimeType, int appProvidedDirect, int appProvidedApp, boolean isWorkprofile, int previewType, String intent) {
        FrameworkStatsLog.write(259, ChooserActivityLogger.SharesheetStartedEvent.SHARE_STARTED.getId(), packageName, getInstanceId().getId(), mimeType, appProvidedDirect, appProvidedApp, isWorkprofile, typeFromPreviewInt(previewType), typeFromIntentString(intent));
    }

    @Override // com.android.internal.app.ChooserActivityLogger
    public void logShareTargetSelected(int targetType, String packageName, int positionPicked) {
        FrameworkStatsLog.write(260, ChooserActivityLogger.SharesheetTargetSelectedEvent.fromTargetType(targetType).getId(), packageName, getInstanceId().getId(), positionPicked);
    }
