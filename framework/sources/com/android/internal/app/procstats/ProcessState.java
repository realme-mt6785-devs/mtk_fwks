...
public final class ProcessState {
    ...
    public void aggregatePss(ProcessStats.TotalMemoryUseCollection data, long now) {
        ...
        if (havePss2) {
            ...
            while (type2 < processState.mDurations.getKeyCount()) {
                ...
                double newAvg = ((totalMemoryUseCollection.processStatePss[procState2] * totalMemoryUseCollection.processStateSamples[procState2]) + (samples2 * samples)) / (totalMemoryUseCollection.processStateSamples[procState2] + samples);
                ...
                dArr[procState2] = dArr[procState2] + (samples2 * time);
                type2++;
                processState = this;
                ...
                while (ips < NPS) {
                    ...
                    while (NPS2 < NSA) {
                        ...
                        while (ima < NMA) {
                            int ipa = 0;
                            while (ipa < NPA) {
                                int vascreen = sepScreenStates ? 0 : iArr[NPS2];
    ...
    /* JADX WARN: Removed duplicated region for block: B:63:0x015b A[LOOP:3: B:61:0x0155->B:63:0x015b, LOOP_END] */
    /*
