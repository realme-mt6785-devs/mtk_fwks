...
public final class Frame {
    ...
    public static Frame make(float[] whitepoint, float adaptingLuminance, float backgroundLstar, float surround, boolean discountingIlluminant) {
        ...
        float c = ((double) f) >= 0.9d ? MathUtils.lerp(0.59f, 0.69f, (f - 0.9f) * 10.0f) : MathUtils.lerp(0.525f, 0.59f, (f - 0.8f) * 10.0f);
        float d2 = discountingIlluminant ? 1.0f : (1.0f - (((float) Math.exp(((-adaptingLuminance) - 42.0f) / 92.0f)) * 0.2777778f)) * f;
        if (d2 > 1.0d) {
            d = 1.0f;
        } else {
            d = ((double) d2) < 0.0d ? 0.0f : d2;
        }
        float[] rgbD = {(((100.0f / rW) * d) + 1.0f) - d, (((100.0f / gW) * d) + 1.0f) - d, (((100.0f / bW) * d) + 1.0f) - d};
        ...
        return new Frame(n, aw, nbb, nbb, c, f, rgbD, fl, (float) Math.pow(fl, 0.25d), z);
    }
}
