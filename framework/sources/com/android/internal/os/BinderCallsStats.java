...
public class BinderCallsStats implements BinderInternal.Observer {
    ...
    private void dumpLocked(PrintWriter pw, AppIdToPackageMap packageMap, int workSourceUid, boolean verbose) {
        ...
        if (workSourceUid != -1) {
            UidEntry e2 = getUidEntry(workSourceUid);
            entries.add(e2);
        ...
        while (it.hasNext()) {
            ...
            long totalRecordedCallsCount4 = entry.cpuTimeMicros;
            pw.println(String.format("  %10d %3.0f%% %8d %8d %s", Long.valueOf(entry.cpuTimeMicros), Double.valueOf((totalRecordedCallsCount4 * 100.0d) / totalCpuTime), Long.valueOf(entry.recordedCallCount), Long.valueOf(entry.callCount), uidStr));
