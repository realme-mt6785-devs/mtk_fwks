...
public final class SmsApplication {
    ...
    private static IOplusSmsApplication mOplusSmsApplication;
    ...
    private static void defaultSmsAppChanged(Context context) {
        ...
        if (getOplusSmsApplication().oemPackageName() != null) {
            for (String str : getOplusSmsApplication().oemPackageName()) {
                assignExclusiveSmsPermissionsToSystemApp(context, packageManager, appOps, str, true);
            }
        }
        String[] oempackage = DEFAULT_APP_EXCLUSIVE_APPOPS;
        for (String opStr : oempackage) {
    ...
    public static ComponentName getDefaultSmsApplication(Context context, boolean updateIfNeeded) {
        if (getOplusSmsApplication().isUserIdEqualMainUserId()) {
            return getDefaultSmsApplicationAsUser(context, updateIfNeeded, getOplusSmsApplication().getUserId());
        }
    ...
    public static boolean shouldWriteMessageForPackage(String packageName, Context context) {
        if (!getOplusSmsApplication().shouldWriteMessageForPackage(packageName)) {
            return false;
        }
    ...
    public static boolean shouldWriteMessageForPackage(String packageName, Context context, int userId) {
        ...
        try {
            Class clazz = Class.forName("com.mediatek.internal.telephony.MtkSmsApplication");
            Class[] argTypes = {String.class, Context.class, Integer.TYPE};
            Method m = clazz.getDeclaredMethod("shouldWriteMessageForPackage", argTypes);
            Object[] params = {packageName, context, Integer.valueOf(userId)};
            Log.d(LOG_TAG, "invoke redirect to " + clazz.getName() + MediaMetrics.SEPARATOR + m.getName());
            return ((Boolean) m.invoke(null, params)).booleanValue();
        } catch (Exception e) {
            Log.w(LOG_TAG, "No MtkSmsApplication! Do nothing. - " + e);
            return shouldWriteMessageForPackage(packageName, context);
        }
    }
    ...
    public static boolean isDefaultSmsApplication(Context context, String packageName, int userId) {
        ...
        try {
            Class clazz = Class.forName("com.mediatek.internal.telephony.MtkSmsApplication");
            Class[] argTypes = {Context.class, String.class, Integer.TYPE};
            Method m = clazz.getDeclaredMethod("isDefaultSmsApplication", argTypes);
            Object[] params = {context, packageName, Integer.valueOf(userId)};
            Log.d(LOG_TAG, "invoke redirect to " + clazz.getName() + MediaMetrics.SEPARATOR + m.getName());
            return ((Boolean) m.invoke(null, params)).booleanValue();
        } catch (Exception e) {
            Log.w(LOG_TAG, "No MtkSmsApplication! Do nothing. - " + e);
    ...
    private static IOplusSmsApplication getOplusSmsApplication() {
        if (mOplusSmsApplication == null) {
            mOplusSmsApplication = OplusMirrorSmsApplication.getInstance.call(new Object[0]);
        }
        return mOplusSmsApplication;
    }
}
