...
class AsyncDrawableCache {
    ...
    public void preloadRes(final Context context, Resources res) {
        ...
        if (isEnableFeature() && isUserUnlocked(context) && (prefs = context.getSharedPreferences(sPerfName, 0)) != null && prefs.getAll().size() != 0) {
            isPreloaded = true;
    ...
    public void putCacheList(long key, Drawable dr, int origResId, Context context) {
        ...
        Drawable.ConstantState boostCache;
        if (isEnableFeature() && !context.getApplicationInfo().processName.equals(CustomProperties.MODULE_SYSTEM) && !context.getApplicationInfo().isSystemApp()) {
            synchronized (lock) {
    ...
    private void clearCacheAfterPreload() {
        if (isPreloaded) {
            ...
            timer.schedule(new TimerTask() { // from class: com.mediatek.res.AsyncDrawableCache.1
                @Override // java.util.TimerTask, java.lang.Runnable
                public void run() {
                    AsyncDrawableCache.this.clearCache();
                    boolean unused = AsyncDrawableCache.isPreloaded = false;
                }
            }, sClearCacheTime);
        }
    }
