...
public class MtkPacketMessage {
    ...
    private static final String TAG = "[MDC]MtkPacketMessage";
    ...
    public void parseFromBytes(byte[] bytes) {
        if (DBG) {
            Log.i(TAG, "parseFromBytes...");
        } else {
            Log.i(TAG, "parseFromBytes " + Arrays.toString(bytes));
        }
        if (bytes.length < 20) {
            Log.e(TAG, "parseFromBytes: too short NLMSG!");
            return;
        }
    ...
    public static byte[] getVerdictMessage(int queueNumber, int packetId, int response) {
        ...
        int totalSize = verdictPrefix.length + VERDICT_SIZE;
